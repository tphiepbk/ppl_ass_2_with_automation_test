...
class BinaryOp(Expr):
    ...
    def __str__(self):
        return "BinaryOp(\'" + self.op.replace('\\', '\\\\') + "\'," + str(self.left) + "," + str(self.right) + ")"
...
class FloatLiteral(Literal):
    ...
    def __str__(self):
        return "FloatLiteral(" + (str(self.value) if str(self.value)!= "inf" else '\'inf\'') + ")"
...
class StringLiteral(Literal):
    ...
    def __str__(self):
        return "StringLiteral(\'" + self.value.replace('\\', '\\\\').replace('\'', '\\\'').replace('\"', '\\\"') + "\')"
